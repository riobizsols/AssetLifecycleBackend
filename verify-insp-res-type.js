...
(async () => {
  try {
    console.log('\n=== CHECKING tblInspResTypeDet TABLE STATUS ===\n');
    
    // Check if table exists
    const tableCheck = await db.query(`
      SELECT table_name 
      FROM information_schema.tables 
      WHERE table_schema = 'public' AND table_name = 'tblInspResTypeDet'
    `);
    
    if (tableCheck.rows.length > 0) {
      console.log('✅ SUCCESS! Table "tblInspResTypeDet" EXISTS\n');
      
      // Get column details
      const columns = await db.query(`
        SELECT column_name, data_type, character_maximum_length, column_default
        FROM information_schema.columns 
        WHERE table_name = 'tblInspResTypeDet' 
      ...
      const foreignKeys = await db.query(`
        ...
        WHERE tc.constraint_type = 'FOREIGN KEY'
          AND tc.table_name = 'tblInspResTypeDet'
      `);
      ...
      const indexes = await db.query(`
        SELECT 
          ...
          indexdef
        FROM pg_indexes 
        WHERE tablename = 'tblInspResTypeDet'
      `);
      ...
      console.log('❌ Table "tblInspResTypeDet" DOES NOT EXIST\n');
    }
