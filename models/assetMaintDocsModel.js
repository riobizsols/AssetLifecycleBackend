...
const insertAssetMaintDoc = async ({
  ...
  const query = `
    INSERT INTO "tblAssetMaintDocs" (
      amd_id, asset_id, doc_type, doc_type_name, doc_path, is_archived, archived_path, org_id
    ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
    RETURNING *
  `;
  const values = [amd_id, asset_id, doc_type, doc_type_name, doc_path, is_archived, archived_path, org_id];
...
const listAssetMaintDocs = async (asset_id) => {
  const query = `
    SELECT amd_id, asset_id, doc_type, doc_type_name, doc_path, is_archived, archived_path, org_id
...
const listAssetMaintDocsByWorkOrder = async (ams_id) => {
  const query = `
    SELECT amd.amd_id, amd.asset_id, amd.doc_type, amd.doc_type_name, amd.doc_path, amd.is_archived, amd.archived_path, amd.org_id
...
// List asset maintenance documents by asset ID and document type
const listAssetMaintDocsByType = async (asset_id, doc_type) => {
  const query = `
    SELECT amd_id, asset_id, doc_type, doc_type_name, doc_path, is_archived, archived_path, org_id
    FROM "tblAssetMaintDocs"
    WHERE asset_id = $1 AND doc_type = $2 AND (is_archived = false OR is_archived IS NULL)
    ORDER BY amd_id DESC
  `;
  return db.query(query, [asset_id, doc_type]);
};

// List asset maintenance documents by work order ID and document type
const listAssetMaintDocsByWorkOrderAndType = async (ams_id, doc_type) => {
  const query = `
    SELECT amd.amd_id, amd.asset_id, amd.doc_type, amd.doc_type_name, amd.doc_path, amd.is_archived, amd.archived_path, amd.org_id
    FROM "tblAssetMaintDocs" amd
    INNER JOIN "tblAssetMaintSch" ams ON amd.asset_id = ams.asset_id
    WHERE ams.ams_id = $1 AND amd.doc_type = $2 AND (amd.is_archived = false OR amd.is_archived IS NULL)
    ORDER BY amd.amd_id DESC
  `;
  return db.query(query, [ams_id, doc_type]);
};
