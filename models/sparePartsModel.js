...
const notifySparePartIssued = async ({ issueRow, org_id, kind = 'reserved' }) => {
  try {
    ...
    const title = isConfirmed ? 'Spare Part Issued' : 'Spare Part Reserved';
    const verb = isConfirmed ? 'issued' : 'reserved';
    ...
    const payload = {
      type: isConfirmed ? 'spare_part_confirmed' : 'spare_part_issued',
      ...
      status: isConfirmed ? 'Issued' : 'Reserved',
      spc_id,
      route: isConfirmed
        ? `/spare-part-issue`
        : `/spare-part-list-detail/${maintenanceId || ''}`,
    };
...
/**
 * In-app notifications for reserved (IS) and issued (IE) spare parts.
...
const getSpareIssuedNotificationsByUser = async ({
  ...
  const result = await dbPool.query(
    `
      ...
      WHERE si.org_id = $1
        AND si.status IN ($${params.length + 1}, $${params.length + 2})
        ${branchFilter}
      ORDER BY COALESCE(si.changed_on, si.created_on) DESC
      LIMIT 100
    `,
    [...params, SPARE_ISSUE_STATUS.RESERVED, SPARE_ISSUE_STATUS.ISSUED],
  );
  ...
  for (const row of result.rows) {
    ...
    rows.push({
      ...
      status_label: row.status === SPARE_ISSUE_STATUS.ISSUED ? 'Issued' : 'Reserved',
    });
  }
  return rows;
};
...
const getSpBrands = async (org_id) => {
  const dbPool = getDb();
  await ensureSpBrandModelSchema(dbPool);
  ...
  const selectSql = `
    SELECT ${quoteIdent(idCol)} AS spb_id, ${quoteIdent(nameCol)} AS text
    FROM "tblSPBrand"
    WHERE 1=1
      ${statusFilter}
  `;

  let result = columns.has('org_id')
    ? await dbPool.query(
        `${selectSql} AND (org_id = $1 OR org_id IS NULL) ORDER BY ${quoteIdent(nameCol)} ASC`,
        [org_id]
      )
    : await dbPool.query(`${selectSql} ORDER BY ${quoteIdent(nameCol)} ASC`);

  if (!result.rows.length && columns.has('org_id')) {
    result = await dbPool.query(
      `${selectSql} ORDER BY ${quoteIdent(nameCol)} ASC`
    );
  }
  return result.rows;
};
...
const getSpModels = async (org_id, spb_id) => {
  const dbPool = getDb();
  await ensureSpBrandModelSchema(dbPool);
  const params = [org_id];
  let query = `
  ...
  if (spb_id) {
    params.push(spb_id);
    query += ` AND spb_id = $${params.length}`;
  }
...
const createCategoryMapping = async ({
  ...
  try {
    ...
    if (!category_brand_id) {
      const err = new Error('Category brand is required');
      err.statusCode = 400;
      throw err;
    }
    if (!category_model_id) {
      const err = new Error('Category model is required');
      err.statusCode = 400;
      throw err;
    }
    if (!asset_type_id) {
    ...
    if (!assetBrandVal) {
      const err = new Error('Asset type brand is required');
      err.statusCode = 400;
      throw err;
    }
    if (!assetModelVal) {
      const err = new Error('Asset type model is required');
      err.statusCode = 400;
      throw err;
    }
    ...
    const modcat = await client.query(
      `
        ...
        LIMIT 1
      `,
      [spc_id, category_model_id, category_brand_id, org_id]
    );
    ...
    if (!spbm_id) {
      ...
      const spModel = await client.query(
        ...
        [
          ...
          category_model_id,
          category_brand_id,
          spBrand.rows[0]?.spb_id || detail.spb_id,
        ]
      );

      if (!spBrand.rows.length || !spModel.rows.length) {
      ...
      const brandName = spBrand.rows[0]?.text || detail.brand_name;
      const modelName = spModel.rows[0]?.text || detail.model_name;
    ...
    const prod_serv_id = prodServ.rows[0].prod_serv_id;
...
const getLotCategoriesByVendor = async (
  ...
  const dbPool = getDb();
  if (!vendor_id) {
    return getCategories(org_id, branch_id, hasSuperAccess, true, true);
  }
...
/**
 * Resolve part number from tblISPPartNumberSpec (+ vendor map when available).
 */
