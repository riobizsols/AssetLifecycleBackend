...
const getAssetsByAssetType = async (
  asset_type_id,
  orgId = null,
...
const getPrinterAssets = async (orgId, branchId, hasSuperAccess = false) => {
  ...
  try {
    ...
    if (orgSettingResult.rows.length > 0 && orgSettingResult.rows[0].value) {
      ...
      console.log(
        "No printer_asset_type setting found in org settings, returning empty result",
      );
      return { rows: [] };
    }
    ...
    console.log(
      "Query executed successfully, found printer assets:",
      result.rows.length,
    );
    return result;
  } catch (error) {
...
const getInactiveAssetsByAssetType = async (
  asset_type_id,
  orgId,
  branchId,
  assignmentType = null,
  ...
  const result = await dbPool.query(query, params);
  console.log(
    "Query executed successfully, found inactive assets:",
    result.rows.length,
  );
  return result;
};
...
const updateAsset = async (
  ...
  try {
    ...
    if (properties && Object.keys(properties).length > 0) {
      ...
      await client.query(
        'DELETE FROM "tblAssetPropValues" WHERE asset_id = $1',
        [asset_id],
      );
      ...
      for (const [propertyName, value] of Object.entries(properties)) {
        ...
        `;
        const propResult = await client.query(propQuery, [
          propertyName,
          asset_type_id,
        ]);
...
const insertAssetPropValue = async (propValueData) => {
  const { asset_id, org_id, asset_type_prop_id, value } = propValueData;
...
const deleteAsset = async (asset_id) => {
  ...
  try {
    ...
    await client.query('DELETE FROM "tblAssetPropValues" WHERE asset_id = $1', [
      asset_id,
    ]);
...
const deleteMultipleAssets = async (asset_ids) => {
  ...
  try {
    ...
    // Delete asset property values first
    await client.query(
      'DELETE FROM "tblAssetPropValues" WHERE asset_id = ANY($1::text[])',
      [asset_ids],
    );
...
const getPotentialParentAssets = async (asset_type_id) => {
  ...
  if (
    parentTypeResult.rows.length === 0 ||
    !parentTypeResult.rows[0].parent_asset_type_id
  ) {
...
// Supports super access users who can view all branches
const getAssetsByExpiryDate = async (
  filterType,
  value = null,
  orgId,
  branchId = null,
...
let finalSerialNumber = serial_number;
    ...
    if (!finalSerialNumber || finalSerialNumber === "") {
      throw new Error(
        "Serial number is required. Please generate a serial number first.",
      );
    }
    ...
    if (finalSerialNumber && finalSerialNumber.length >= 5) {
      ...
      if (!isNaN(sequenceNumber)) {
        ...
        console.log(
          `🔢 Updated last_gen_seq_no to ${sequenceNumber} for asset type ${asset_type_id}`,
        );
      }
    }
...
const getAssetsByUserContext = async (
  orgId,
  branchId = null,
  dbConnection = null,
  hasSuperAccess = false,
  ...
  try {
    ...
    const [organizations, assetTypes, branches, vendors, prodServs] =
      await Promise.all([
        ...
        dbPool.query(
          'SELECT asset_type_id, text as asset_type_name FROM "tblAssetTypes" WHERE int_status = 1',
        ),
        dbPool.query(
          'SELECT branch_id, text as branch_name FROM "tblBranches" WHERE int_status = 1',
        ),
        dbPool.query(
          'SELECT vendor_id, text as vendor_name FROM "tblVendors" WHERE int_status = 1',
        ),
        dbPool.query(
...
const validatePropertyValue = async (
  client,
  assetTypePropId,
  value,
  orgId,
  ...
  const propIdResult = await client.query(propIdQuery, [
    assetTypePropId,
    orgId,
  ]);
  ...
  const caseInsensitiveMatch = existingValues.find(
    (row) => row.value.toLowerCase() === trimmedValue.toLowerCase(),
  );
...
const validatePropertyValueStandalone = async (
  assetTypePropId,
  value,
  orgId,
  createdBy = "system",
  ...
  const dbPool = getDb();
  const propIdResult = await dbPool.query(propIdQuery, [
    assetTypePropId,
    orgId,
  ]);
...
const validateAndFormatDate = (dateString) => {
  ...
  if (ddMMyyyyRegex.test(dateString)) {
    ...
    if (isNaN(date.getTime())) {
      throw new Error(
        `Invalid date format: ${dateString}. Expected DD-MM-YYYY or YYYY-MM-DD`,
      );
    }
...
const bulkUpsertAssets = async (
  ...
  try {
    ...
    const assetTypesResult = await client.query(
    ...
    for (const row of csvData) {
      ...
      try {
        ...
        if (!finalSerialNumber) {
          ...
          const formattedAssetTypeId = convertAssetTypeToSerialFormat(
            row.asset_type_id,
          );
          finalSerialNumber = `${formattedAssetTypeId}${reversedYear}${month}${String(nextSequence).padStart(5, "0")}`;
          console.log(
            `🔢 Generated serial number for bulk upload (tx-safe): ${finalSerialNumber}`,
          );
        }
        ...
        const lastDepreciationCalcDate = validateAndFormatDate(
          row.last_depreciation_calc_date,
        );
        ...
        if (row.properties && Object.keys(row.properties).length > 0) {
          console.log(
            "Saving property values for asset:",
            finalAssetId,
            row.properties,
          );
          ...
          if (propertyValidationErrors.length > 0) {
            throw new Error(
              `Property validation failed: ${propertyValidationErrors.join("; ")}`,
            );
          }
          ...
          for (const [assetTypePropId, value] of Object.entries(
            row.properties,
          )) {
            if (value && value.trim() !== "") {
