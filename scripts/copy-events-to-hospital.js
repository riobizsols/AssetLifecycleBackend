...
/**
 * Copy Events Data Script
 * 
 * This script copies data from tblEvents
 * from the generic database to the hospital database.
 * 
 * Usage:
 *   node scripts/copy-events-to-hospital.js <generic-db-name> <hospital-db-name>
 * 
 * Example:
 *   node scripts/copy-events-to-hospital.js assetLifecycle hospital
 */
...
async function main() {
  ...
  if (args.length < 2) {
    console.error('❌ Usage: node scripts/copy-events-to-hospital.js <generic-db-name> <hospital-db-name>');
    console.error('   Example: node scripts/copy-events-to-hospital.js assetLifecycle hospital');
    process.exit(1);
  }
  ...
  console.log('🚀 Starting Events Data Copy');
  ...
  try {
    ...
    const existsInGeneric = await checkTable(genericClient, genericDbName, tableName);
    const existsInHospital = await checkTable(hospitalClient, hospitalDbName, tableName);
    
    if (!existsInGeneric) {
      throw new Error(`Table ${tableName} does not exist in ${genericDbName}`);
    }
    if (!existsInHospital) {
      throw new Error(`Table ${tableName} does not exist in ${hospitalDbName}`);
    }
    console.log(`   ✅ ${tableName} exists in both databases`);
    ...
    const eventsResult = await copyTableData(
      genericClient,
      hospitalClient,
      'tblEvents',
      eventsColumns
    ...
    console.log('═'.repeat(60));
    console.log(`\n📋 tblEvents:`);
    console.log(`   ✅ Copied: ${eventsResult.copied}`);
    console.log(`   ⏭️  Skipped: ${eventsResult.skipped}`);
    console.log(`   ❌ Errors: ${eventsResult.errors}`);
    console.log('═'.repeat(60));
    
    if (eventsResult.errors > 0) {
