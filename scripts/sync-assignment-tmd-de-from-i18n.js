...
const TARGET_FILES = [
  ...
  path.join(FRONTEND_ROOT, "components", "assetAssignment", "AssetAssignmentHistory.jsx"),
];
...
async function run() {
  ...
  if (!dedup.length) {
    console.log("No assignment i18n-backed tmd rows found.");
    return;
  }
  ...
  try {
    ...
    console.log(`Updated German text for ${dedup.length} assignment-flow tmd_id rows.`);
  } catch (error) {
