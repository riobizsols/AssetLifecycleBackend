/**
 * Add CNC Trainer Machine technician certificate + technician for NGP (AT009).
 *
 * Usage: node scripts/seed-ngp-cnc-technician.js
 */
...
const ORG = 'ORG003';
const USER = 'USR001';
const ASSET_TYPES = ['AT009']; // CNC Trainer Machine
const CERT_NAME = 'CNC Trainer Maintenance Certificate';
const CERT_NO = 'CNC-MAINT-001';
const TECH_NAME = 'CNC Field Technician';
const TECH_EMAIL = 'ngp.cnc.technician@ngp';
...
function nextId(prefix, maxId) {
  ...
  const width = Math.max(
    prefix === 'EMP_INT_' ? 4 : 3,
    String(maxId || '').slice(prefix.length).length || 3,
  );
...
(async () => {
  ...
  try {
    ...
    // Resolve asset type ids by name as well (in case id differs)
    const typeRows = await client.query(
      `SELECT asset_type_id, text FROM "tblAssetTypes"
       WHERE org_id = $1
         AND (
           asset_type_id = ANY($2::text[])
           OR LOWER(TRIM(text)) = LOWER('CNC Trainer Machine')
         )
       ORDER BY asset_type_id`,
      [ORG, ASSET_TYPES],
    );
    if (!typeRows.rows.length) {
      throw new Error('CNC Trainer Machine asset type not found for ORG003');
    }
    console.log(
      'Asset types:',
      typeRows.rows.map((r) => `${r.asset_type_id} ${r.text}`).join(', '),
    );
    ...
    if (!empIntId) {
      ...
      await client.query(
        `INSERT INTO "tblEmployees" (
           ...
           $1,$2,$3,'CNC','Technician',$3,
           $4,'9876500124','PERMANENT', CURRENT_DATE, 'en', 1,
           $5,'BR002','DPT003',
           $6,$7,$6,$7
         )`,
    ...
    const existingEtc = await client.query(
      `SELECT etc_id FROM "tblEmpTechCert" WHERE emp_int_id = $1 AND tc_id = $2 LIMIT 1`,
      [empIntId, tcId],
    );
    ...
    for (const row of typeRows.rows) {
      ...
      const required = await client.query(
        `SELECT DISTINCT tc_id FROM "tblATMaintCert"
         WHERE asset_type_id=$1 AND (org_id=$2 OR org_id IS NULL)`,
        [assetTypeId, ORG],
      );
      ...
      const techs = await client.query(
        ...
        SELECT e.emp_int_id, e.full_name, e.email_id, MIN(tc.certificate_name) AS cert_name
      ...
      console.log(`\nCertified for ${assetTypeId}:`, techs.rows);
    }
  } catch (err) {
    try {
